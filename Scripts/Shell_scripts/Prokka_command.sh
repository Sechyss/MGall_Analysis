...
# This code runs in the folder with all the assemblies
...
for accession_id in "${runs[@]}"; do
  prokka --force --outdir "$accession_id"_prokka --prefix "$accession_id" --kingdom Bacteria --gcode 4 --locustag "$accession_id" --genus 'Mycoplasma' --species 'gallisepticum' --usegenus --rnammer "$accession_id"_assembly/contigs.fasta; done