#!/bin/bash
#
# SPADES_Pipeline.sh — De novo genome assembly of SRA reads with SPAdes.
#
# Runs SPAdes for each SRA accession using quality-trimmed paired-end reads
# (R1, R2) and singleton reads produced by Sickle_readTrimming.sh.
# Output assemblies are written to per-sample directories.
#
# Usage:
#   bash SPADES_Pipeline.sh
#   (Update the base read/assembly paths before running)
#
# Requirements: SPAdes (>=3.15)
# Input format:  <accession>_sickle_R1.fastq, _R2.fastq, _single.fastq
