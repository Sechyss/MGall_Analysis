...
def SEIRS_second_model(y, t, params):
    """
    SEIRS_second_model
    - Same signature as SEIRS_first_model.
    - In this variant treated low infecteds (Idl) DO contribute to transmission:
      B_l = beta_l * (Indl + Idl)
    - Contains same numeric safeguards and validations as the first model.
    """
    y = np.maximum(np.asarray(y, dtype=float), 0.0)
    S, Eh, Indh, Idh, Rh, El, Indl, Idl, Rl = y
    ...
    beta_h = phi_transmission * beta_l
    B_h = beta_h * (Indh + Idh)
    # treated low infecteds ARE infectious in this variant
    B_l = beta_l * (Indl + Idl)
    ...
    total = S + Eh + Indh + Idh + Rh + El + Indl + Idl + Rl
    if not np.isfinite(total) or total <= 0:
        raise RuntimeError("Non-finite or non-positive total population in SEIRS_second_model")
