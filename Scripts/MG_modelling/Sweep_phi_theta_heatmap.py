...
# storage arrays: shape (len(theta), len(phi_transmission))
# peak_high: maximum prevalence of high-virulence infection over the simulation (peak)
peak_high = np.zeros((len(theta), len(phi_transmission)))
...
for i, theta in enumerate(theta):
    for j, phi in enumerate(phi_transmission):
...
def plot_heatmap(ax, data, title, cmap='viridis'):
    ...
    im = ax.imshow(data, aspect='auto', origin='lower', cmap=cmap,
                   extent=[phi_transmission[0], phi_transmission[-1], theta[0], theta[-1]])
...
df = pd.DataFrame({
    'theta': np.repeat(theta, len(phi_transmission)),               # repeat theta for each phi
    'phi_transmission': np.tile(phi_transmission, len(theta)),      # tile phi across theta rows
